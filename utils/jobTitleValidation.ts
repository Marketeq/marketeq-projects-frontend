...
export const categoryCheck = async (input: string): Promise<{ valid: boolean; category: string | null }> => {
...
export const assignSemanticCategory = async (label: string): Promise<string | null> => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_SEMANTIC_API_URL}/semantic-category`, {
      method: "POST",
