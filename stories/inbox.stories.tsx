...
export const Bubble = ({ message, currentUser,
  participantUsername,}: { message: Message 
  ...
  const senderUsername = isMe ? "@" + currentUser.username : "@" + participantUsername
...
useEffect(() => {
  ...
  const handleDeleteMessage = async () => {
    try {
      ...
      await apiFetch(
        `/api/messaging/messages/${message.id}/delete`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${jwtToken}` },
        }
  ...
  const handleRestoreMessage = async () => {
    try {
      ...
      await apiFetch(
        `/api/messaging/messages/${message.id}/undo-delete`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${jwtToken}` },
        }
...
export const YourBubble =  ({ message, currentUser,
  participantUsername,}: { message: Message 
     currentUser:  CurrentUser,
     participantUsername?: string
   }) => {
...
const dummyConv: Conversation & { participantUsername: string } = {
  ...
  participants: [
    { conversationId: 'conv-1', userId: 'user-1', joinedAt: new Date().toISOString() },
    { conversationId: 'conv-1', userId: 'user-2', joinedAt: new Date().toISOString() },
  ],
