...
import { createTestUser, loginViaUI } from "tests/helpers/authHelper"
import { getTestUser } from "tests/helpers/testUser"
...
Before(async function ({ pickle }: any) {
  if (!loggerInitialized) {
    const logPath = path.resolve(process.cwd(), "tests/reports/execution.log")
  ...
  // Run headed by default, headless when HEADED=false
  const headless = process.env.HEADED == "false"
  ...
  logger.info(
    `[${featureUri}] Scenario "${pickle?.name ?? "unknown"}" has ${totalSteps} executable steps`
  )
})

let testUserCreated = false

// ─── Creates random user once via API for @requiresUser scenarios ──────
Before({ tags: "@requiresUser" }, async function (this: any) {
  if (!testUserCreated) {
...
// ─── Logs in via UI using the randomly created user credentials ────────
Before({ tags: "@requiresAuth" }, async function (this: any) {
  const user = getTestUser()
  await loginViaUI(this.page, user.email, user.password)
})

BeforeStep(async function (this: any, stepInfo: any) {
  const id = String(stepInfo?.testStepId ?? "unknown-step")
  const stepText =
    stepInfo?.pickleStep?.text ??
    stepInfo?.gherkinDocument?.feature?.name ??
    "unknown step"
  stepTimers.set(id, Date.now())
  logger.info(
    `[${this.currentFeatureUri ?? "unknown-feature"}] STEP START: ${stepText}`
  )
})
...
AfterStep(async function (this: any, stepInfo: any) {
  ...
  logger.info(
    `[${this.currentFeatureUri ?? "unknown-feature"}] STEP END: ${stepText} | status=${status} | duration=${elapsedMs}ms`
  )
})

After(
  { tags: "not @skipAfterHook" },
  async function (this: any, scenario: any) {
    ...
    try {
      ...
      if (this.page && !this.page.isClosed()) {
        ...
        logger.info(
          `Screenshot captured, size: ${finalScreenshot.length} bytes`
        )
        ...
        const filepath = path.join(
          screenshotsDir,
          `${scenarioName}-${status}-${timestamp}.png`
        )
