...
Given(
  "featured image fixture is available for project publishing",
...
Given(
  ...
  async function (this: any, pageName: string) {
...
Then(
  ...
  async function (this: any, headingText: string) {
...
When(
  "I fill the project title as {string}",
  async function (this: any, title: string) {
    await getPublishProjectPage(this).fillProjectTitle(title)
  }
)

When(
  "I select project categories:",
  async function (this: any, dataTable: DataTable) {
...
When("I upload the featured image file", async function (this: any) {
  ...
  const hasButton = await uploadButton
...
When("I click Edit Phase Name", async function (this: any) {
  ...
  const isVisible = await phaseNameButton
    .isVisible({ timeout: 2000 })
    .catch(() => false)
  if (isVisible) {
    ...
    for (let i = 0; i < buttonCount; i++) {
      ...
      const hasAriaExpanded = await btn
...
When(
  "I enter phase name as {string}",
  async function (this: any, phaseName: string) {
...
When(
  "I add a project task with name {string} for the phase",
  async function (this: any, taskName: string) {
...
When(
  "I add a project phase with name {string}",
  async function (this: any, phaseName: string) {
    await getPublishProjectPage(this).addPhase()
...
When(
  "I add a project task with name {string}",
  async function (this: any, taskName: string) {
...
When("I select role as {string}", async function (this: any, roleName: string) {
  const roleCell = this.page
    .getByRole("button", { name: /select role/i })
    .first()
  const canEditInDialog = await roleCell
    .isVisible({ timeout: 2000 })
    .catch(() => false)
  ...
  await expect(
    this.page.getByText(roleName, { exact: false }).first()
  ).toBeVisible({ timeout: 10000 })
})

When(
  "I select role as {string} for the task",
  async function (this: any, roleName: string) {
    ...
    const roleTrigger = taskDialog
      ...
      .first()
    const isVisible = await roleTrigger
      .isVisible({ timeout: 2000 })
      .catch(() => false)
...
When(
  "I select location as {string}",
  async function (this: any, locationName: string) {
    const locationCell = this.page
      .getByText("Select country", { exact: false })
      .first()
    const canEditInDialog = await locationCell
      .isVisible({ timeout: 2000 })
      .catch(() => false)
    ...
    await expect(
      this.page.getByText(locationName, { exact: false }).first()
    ).toBeVisible({ timeout: 10000 })
  }
)

When(
  "I select location as {string} for the task",
  async function (this: any, locationName: string) {
    ...
    const locationTrigger = taskDialog
      ...
      .first()
    const isVisible = await locationTrigger
      .isVisible({ timeout: 2000 })
      .catch(() => false)
...
When(
  "I select experience level as {string}",
  async function (this: any, experienceLevel: string) {
    const experienceCell = this.page
      .getByText("Select experience", { exact: false })
      .first()
    const canEditInDialog = await experienceCell
      .isVisible({ timeout: 2000 })
      .catch(() => false)

    if (canEditInDialog) {
      await experienceCell.click()
      const experienceOption = this.page.getByText(experienceLevel, {
        exact: false,
      })
...
When(
  "I select experience level as {string} for the task",
  async function (this: any, experienceLevel: string) {
    ...
    const experienceTrigger = taskDialog
      ...
      .first()
    const isVisible = await experienceTrigger
      .isVisible({ timeout: 2000 })
      .catch(() => false)
    ...
    throw new Error(
      `Experience level selection button not found or not visible`
    )
  }
)

When(
  "I set duration as {string} {string}",
  async function (this: any, value: string, type: string) {
    const durationInput = this.page
      .locator('input[id="duration"]')
      .or(this.page.locator('input[name="duration.value"]'))
    const canEditInDialog = await durationInput
      .first()
      .isVisible({ timeout: 2000 })
      .catch(() => false)
    ...
    if (canEditInDialog) {
      ...
      if (type.toLowerCase() !== "days") {
        const typeButton = this.page
          .getByRole("button", { name: /days|weeks|hours/i })
          .first()
    ...
    await expect(this.page.getByText(expectedDuration).first()).toBeVisible({
      timeout: 10000,
    })
  }
)
...
When(
  "I select duration as {string} for the task",
  async function (this: any, durationText: string) {
    console.log(`⏱️  Selecting duration for task: ${durationText}`)
    // Handle format like "1-3 months"
    const taskDialog = this.page.locator('[role="dialog"]').last()
    const durationInput = taskDialog
      .locator('input[id="duration"], input[name="duration.value"]')
      .first()
    const isVisible = await durationInput
      .isVisible({ timeout: 2000 })
      .catch(() => false)
    ...
    if (isVisible) {
      ...
      if (match) {
        ...
        if (type.toLowerCase() !== "days") {
          const typeButton = taskDialog
            .getByRole("button", { name: /days|weeks|months|hours/i })
            .first()
          await typeButton.click()
          await this.page.waitForTimeout(300)
    ...
    throw new Error(
      `Duration input not found or invalid format: ${durationText}`
    )
  }
)

When(
  "I select duration as {string} {string} for the task",
  async function (this: any, value: string, type: string) {
    ...
    const isInputVisible = await durationInput
      .isVisible({ timeout: 2000 })
      .catch(() => false)
    ...
    const singularType = type.toLowerCase().endsWith("s")
      ? type.slice(0, -1)
      : type
    const displayType =
    ...
    if (displayType.toLowerCase() !== "day") {
      ...
      const isButtonVisible = await durationTypeButton
        .isVisible({ timeout: 2000 })
        .catch(() => false)
      ...
      if (isButtonVisible) {
        ...
        const typeOption = this.page
          .getByRole("menuitem")
          .filter({ hasText: new RegExp(displayType, "i") })
          .first()
        const hasOption = await typeOption
          .isVisible({ timeout: 2000 })
          .catch(() => false)
        ...
        if (hasOption) {
          ...
          const altOption = this.page
            .getByText(displayType, { exact: false })
            .first()
...
Then(
  ...
  async function (this: any, message: string) {
    console.log(`✅ Verifying success message: "${message}"`)
    console.log(
      `📍 Current URL: ${await getPublishProjectPage(this).getPageUrl()}`
    )
    ...
    try {
      ...
      throw new Error(
        `Success message "${message}" not found. Page content: ${allText.substring(0, 500)}`
      )
    }
  }
)
...
Then(
  ...
  async function (this: any, messageText: string) {
